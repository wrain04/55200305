...
public interface ProductUserDetailsService extends UserDetailsService, Ordered {
	...
	default UserDetails getUserDetails(R<UserInfo> result) {
		...
		// 构造security用户
		return new ProductUser(user.getUserId(), user.getDeptId(), user.getUsername(),
				SecurityConstants.BCRYPT + user.getPassword(), user.getPhone(), user.getOpenid(), true, true, true,
	 ...
	 * @return
	 */
	default UserDetails loadUserByUser(ProductUser productUser) {
		return this.loadUserByUsername(productUser.getUsername());
	}
