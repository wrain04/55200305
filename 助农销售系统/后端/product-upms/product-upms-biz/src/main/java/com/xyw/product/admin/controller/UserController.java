...
import com.xyw.product.admin.api.vo.UserExcelVO;
import com.xyw.product.admin.api.vo.UserInfoVO;
...
import com.xyw.product.common.xss.core.XssCleanIgnore;
...
public class UserController {
	...
	public R<UserInfoVO> info() {
		...
		SysUser user = userService.getOne(Wrappers.<SysUser>query().lambda().eq(SysUser::getUserId, userId));
		if (user == null) {
	...
	@Inner(false)
	public R<UserVO> usernameAndAvatar(@PathVariable Long id) {
	...
	@Inner
	@PostMapping("/wechat")
	public R<Boolean> wechatUser(@RequestBody UserDTO userDto) {
		return R.ok(userService.saveUser(userDto));
	}
	...
	@Operation(summary = "查询用户是否是商户", description = "查询用户是否是商户")
	@SysLog("查询用户是否是商户")
	@GetMapping("/isBusiness/{id:\\d+}")
	R<Boolean> isBusiness(@PathVariable("id") Long id) {
		return R.ok(userService.isBusiness(id));
	}

	@Operation(summary = "查询自己是否是商户", description = "查询自己是否是商户")
	@SysLog("查询自己是否是商户")
	@GetMapping("/checkBusiness")
	R<Boolean> checkBusiness() {
		return R.ok(userService.isBusiness(SecurityUtils.getUser().getId()));
	}
